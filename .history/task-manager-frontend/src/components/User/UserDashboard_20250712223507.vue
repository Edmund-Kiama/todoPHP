<script setup>
  ...
  const activeTasks = computed(() => userTasks.filter(t => t.status !== 'Completed'))
const completedTasks = computed(() => userTasks.filter(t => t.status === 'Completed'))
...
<template>
  ...
  <section>
    ...
    <!-- Active Tasks Table -->
    <table v-if="activeTasks.length">
      <thead>
        <tr>
          <th>Task</th><th>Deadline</th><th>Status</th><th>Action</th>
        </tr>
      </thead>
      <tbody>
